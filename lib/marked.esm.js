...
const getCounter$1 = () => memoCounter;

const memoize$2 = (func) => {
  const results = {};
  return (...args) => {
...
var memoize_1 = { memoize: memoize$2, getCounter: getCounter$1 };

const { memoize: memoize$1 } = memoize_1;
...
const cleanUrl$1 = memoize$1((sanitize, base, href) => {
  if (sanitize) {
    let prot;
    try {
...
const splitCells$1 = memoize$1((tableRow, count) => {
...
const rtrim$1 = memoize$1((str, c, invert) => {
  const l = str.length;
  if (l === 0) {
    return "";
  }
...
const findClosingBracket$1 = memoize$1((str, b) => {
...
const { merge, checkSanitizeDeprecation, escape } = helpers;
const { getDefaults, changeDefaults, defaults } = defaults$5.exports;
const { getCounter } = memoize_1;
